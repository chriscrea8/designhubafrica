...
import { ArrowLeft, CheckCircle2, Clock, DollarSign, MessageSquare, AlertTriangle, MapPin, Star, XCircle, Loader2, Upload, CreditCard, Lock, Unlock, FileText } from "lucide-react";

function ContractLinkButton({ projectId }: { projectId: string }) {
  const [contractId, setContractId] = React.useState<string | null>(null);
  React.useEffect(() => {
    fetch("/api/contracts").then(r => r.json()).then(res => {
      if (res.success) {
        const c = (res.data || []).find((c: any) => c.projectId === projectId);
        if (c) setContractId(c.id);
      }
    }).catch(() => {});
  }, [projectId]);
  if (!contractId) return null;
  return <Link href={`/contracts/${contractId}`} className="flex items-center justify-center gap-2 w-full h-9 rounded-md border border-input bg-background px-3 text-sm font-medium hover:bg-accent transition-colors"><FileText className="h-3.5 w-3.5" />View Contract</Link>;
}
...
function ProjectDetailContent() {
  ...
  async function handleRevise(milestoneId: string) {
    if (!revisionReason || revisionReason.length < 10) { alert("Please describe the revision needed (min 10 chars)"); return; }
    setActionLoading(milestoneId);
    const res = await fetch(`/api/milestones/${milestoneId}/revise`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ reason: revisionReason }) });
    const json = await res.json();
    setActionLoading(null);
    if (json.success) { setShowRevisionModal(null); setRevisionReason(""); loadProject(); }
    else alert(json.error || "Failed");
  }

  async function handleDispute(milestoneId: string) {
    if (!disputeReason || disputeReason.length < 20) { alert("Please describe the dispute clearly (min 20 chars)"); return; }
    setActionLoading(milestoneId);
    const res = await fetch(`/api/milestones/${milestoneId}/dispute`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ reason: disputeReason }) });
    const json = await res.json();
    setActionLoading(null);
    if (json.success) { setShowDisputeModal(null); setDisputeReason(""); loadProject(); alert(json.data?.message); }
    else alert(json.error || "Failed");
  }
  ...
  async function submitReview() {
    ...
    if (reviewForm.comment.length < 10) { alert("Please write at least 10 characters"); return; }
    const res = await fetch("/api/reviews", {
      ...
      body: JSON.stringify({
        ...
        qualityRating: (reviewForm as any).qualityRating || reviewForm.rating,
        communicationRating: (reviewForm as any).communicationRating || reviewForm.rating,
        timelinessRating: (reviewForm as any).timelinessRating || reviewForm.rating,
        professionalismRating: (reviewForm as any).professionalismRating || reviewForm.rating,
      }),
    });
    const json = await res.json();
    if (json.success) { setShowReview(false); alert("Review submitted! Thank you."); loadProject(); }
    else alert(json.error || "Failed to submit review");
  }
    ...
    <div className="space-y-6">
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2">
          {tab === "overview" && <Card><CardHeader><CardTitle className="text-base">Description</CardTitle></CardHeader><CardContent><p className="text-sm text-muted-foreground whitespace-pre-wrap">{project.description}</p>{project.images?.length > 0 && (<div className="mt-4"><p className="text-sm font-medium mb-2">Inspiration Photos</p><div className="grid grid-cols-2 sm:grid-cols-3 gap-3">{project.images.map((img: string, i: number) => <img key={i} src={img} alt={`Inspiration ${i+1}`} className="rounded-lg aspect-video object-cover w-full" />)}</div></div>)}</CardContent></Card>}
                  ...
                  <div className="flex-1">
                    ...
                    {isClient && ["submitted", "completed"].includes(ms.status) && (
                      <div className="flex gap-2 mt-3 flex-wrap">
                        <Button variant="terracotta" size="sm" className="gap-1.5" onClick={() => approveMilestone(ms.id)} disabled={!!actionLoading}>{actionLoading === `approve-${ms.id}` ? <Loader2 className="h-3 w-3 animate-spin" /> : <CheckCircle2 className="h-3 w-3" />}Approve & Release</Button>
                        <Button variant="outline" size="sm" className="gap-1.5" onClick={() => setShowRevisionModal(ms.id)}><XCircle className="h-3 w-3" />Request Revision</Button>
                        <Button variant="outline" size="sm" className="gap-1.5 text-red-500 border-red-200 hover:bg-red-50" onClick={() => setShowDisputeModal(ms.id)}><AlertTriangle className="h-3 w-3" />Dispute</Button>
                      </div>
                    )}
                    ...
                    {ms.status === "disputed" && <p className="text-xs text-red-500 mt-2 flex items-center gap-1"><AlertTriangle className="h-3 w-3" />Disputed — admin reviewing</p>}

                    {/* Revision Modal */}
                    {showRevisionModal === ms.id && (
                      <div className="mt-3 p-3 rounded-lg border border-amber-200 bg-amber-50/50 space-y-2">
                        <p className="text-xs font-semibold text-amber-700">Describe what needs to change:</p>
                        <textarea value={revisionReason} onChange={e => setRevisionReason(e.target.value)} rows={3} className="w-full rounded-md border border-input bg-background px-3 py-2 text-xs resize-none" placeholder="e.g. The colour palette doesn't match what we discussed. Please revise the living room section." />
                        <div className="flex gap-2">
                          <Button variant="terracotta" size="sm" onClick={() => handleRevise(ms.id)} disabled={!!actionLoading}>{actionLoading === ms.id ? <Loader2 className="h-3 w-3 animate-spin" /> : null}Send</Button>
                          <Button variant="outline" size="sm" onClick={() => { setShowRevisionModal(null); setRevisionReason(""); }}>Cancel</Button>
                        </div>
                      </div>
                    )}

                    {/* Dispute Modal */}
                    {showDisputeModal === ms.id && (
                      <div className="mt-3 p-3 rounded-lg border border-red-200 bg-red-50/50 space-y-2">
                        <p className="text-xs font-semibold text-red-700">⚠ Filing a dispute will lock the escrow and escalate to admin review (14 days).</p>
                        <textarea value={disputeReason} onChange={e => setDisputeReason(e.target.value)} rows={3} className="w-full rounded-md border border-input bg-background px-3 py-2 text-xs resize-none" placeholder="Describe the issue in detail. Include what was agreed vs what was delivered." />
                        <div className="flex gap-2">
                          <Button size="sm" className="bg-red-500 hover:bg-red-600 text-white" onClick={() => handleDispute(ms.id)} disabled={!!actionLoading}>{actionLoading === ms.id ? <Loader2 className="h-3 w-3 animate-spin" /> : null}File Dispute</Button>
                          <Button variant="outline" size="sm" onClick={() => { setShowDisputeModal(null); setDisputeReason(""); }}>Cancel</Button>
                        </div>
                      </div>
                    )}
                  </div>
        ...
        <div className="space-y-4">
          ...
          {isClient && <Card><CardContent className="p-4 space-y-2">
            <ContractLinkButton projectId={id} />
            <Button variant="outline" className="w-full" size="sm" onClick={() => alert("Dispute filing coming soon")}><AlertTriangle className="h-3.5 w-3.5 mr-2" />Open Dispute</Button>
          </CardContent></Card>}
        </div>
      </div>
