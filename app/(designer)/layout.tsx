...
import { designerNavItems } from "@/lib/constants";
...
export default function Layout({ children }: { children: React.ReactNode }) {
  ...
  const [navItems, setNavItems] = useState(designerNavItems);

  useEffect(() => {
    fetch("/api/notifications").then(r => r.json()).then(res => {
      if (res.success && res.data?.unread > 0) {
        setNavItems(prev => prev.map(item => item.href === "/client-messages" ? { ...item, badge: res.data.unread } : item));
      }
    }).catch(() => {});
  }, []);
