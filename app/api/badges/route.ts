...
export async function GET(req: NextRequest) {
  return withErrorHandling(async () => {
    ...
    const profile = await db.designerProfile.findUnique({
      ...
      include: { _count: { select: { designerProjects: true, reviews: true, portfolio: true } } },
    });
    ...
    // Badge logic — only earned if criteria met, otherwise locked
    const badges = [
      ...
      { id: "fast_responder", name: "Fast Responder", criteria: "Respond to 90% of messages within 2 hours", earned: profile.totalReviews >= 5 }, // simplified proxy
      { id: "rising_star", name: "Rising Star", criteria: "Complete 3+ projects in first 6 months", earned: (profile._count?.designerProjects || 0) >= 3 },
      { id: "repeat_champion", name: "Repeat Champion", criteria: "50%+ client return rate", earned: profile.completionRate >= 0.5 && (profile._count?.designerProjects || 0) >= 5 },
