...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  return withErrorHandling(async () => {
    ...
    const profile = user!.role === "DESIGNER"
      ? await db.designerProfile.findUnique({ where: { userId: user!.id }, select: { id: true, userId: true } })
      : await db.artisanProfile.findUnique({ where: { userId: user!.id }, select: { id: true, userId: true } });
