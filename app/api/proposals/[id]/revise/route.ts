...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  return withErrorHandling(async () => {
    ...
    const proposal = await db.proposal.findUnique({ where: { id: params.id }, include: { project: { select: { id: true, title: true, clientId: true } }, designer: { select: { id: true, userId: true } } } });
