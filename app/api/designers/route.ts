...
export async function GET(req: NextRequest) {
  return withErrorHandling(async () => {
    ...
    if (search) where.user = { ...where.user, firstName: { contains: search, mode: "insensitive" } };
    const [designers, total] = await Promise.all([
      db.designerProfile.findMany({ where, skip, take: limit, orderBy: { avgRating: "desc" },
        include: { user: { select: { id: true, firstName: true, lastName: true, image: true, location: true, isVerified: true } }, _count: { select: { reviews: true, designerProjects: true } } } }),
