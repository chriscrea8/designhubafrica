...
export function DesignerCard({ designer, isSaved, onToggleSave }: { designer: any; isSaved?: boolean; onToggleSave?: () => void }) {
  const profileUrl = `/designers/${designer.id}`;
  const user = designer.user || {};
  return <motion.div whileHover={{ y: -4 }} transition={{ duration: 0.2 }} className="relative"><Link href={profileUrl}><Card className="overflow-hidden group cursor-pointer hover:shadow-md transition-shadow">
  ...
  {/* Save/Heart button — sits outside the Link to prevent navigation */}
  {onToggleSave && (
    <button
      onClick={e => { e.preventDefault(); e.stopPropagation(); onToggleSave(); }}
      className={cn("absolute top-3 right-3 h-8 w-8 rounded-full shadow-md flex items-center justify-center transition-colors z-10", isSaved ? "bg-red-50 border border-red-200" : "bg-white/90 border border-border hover:bg-red-50")}
      title={isSaved ? "Remove from saved" : "Save designer"}
    >
      <Heart className={cn("h-4 w-4", isSaved ? "fill-red-400 text-red-400" : "text-muted-foreground")} />
    </button>
  )}
  </motion.div>;
}
