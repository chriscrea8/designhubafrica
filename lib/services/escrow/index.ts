...
export async function initiatePayout(userId: string, amount: number, description: string) {
  ...
  try {
    ...
    const success = !!transferJson.status;
    ...
    await db.payout.create({
      data: {
        ...
        description,
        failureReason: success ? undefined : (transferJson.message || "Transfer failed"),
      },
    }).catch((e: any) => console.error("[Payout Record Error]", e.message));

    console.log("[Payout] Transfer:", success ? "initiated" : "failed", reference);
    return { reference, status: success ? "pending" : "failed" };
  } catch (e) {
